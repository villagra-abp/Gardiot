...
plant.getPlants = function(number, page, orderBy, sort, callback) {
  if(connection) {
    ...
    connection.query('SELECT Plant.id, family, name, commonName, photo, _3DModel, scientificName, depth, distance, diseaseResist FROM Plant, Family WHERE Plant.family = Family.id ORDER BY ' + orderByParam + ' LIMIT ' + minPeak  + ',' + number , function (error, rows){
      if(error)
        callback(error, null);
      else
        callback(null, rows);
