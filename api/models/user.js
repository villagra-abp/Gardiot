...
user.updateUser = function(userData, callback) { //No funciona porque no conservo el id viejo para el WHERE
	if (connection) {
		var mariasql = 'UPDATE User SET ';
		if (userData.password) //Todo esto es en caso de que no se haga un GET para mostrar los valores anteriores en el formulario
			mariasql += 'password = "' + userData.password + '",';
...
/*user.loginUser = function(userData, callback) {
	if (connection) {
		var sql = 'SELECT COUNT(id, password) FROM User WHERE id = ' + userData.id + ' and password = "' + userData.password + '"';
		connection.query(sql, function(error, result) {
			if (error)
				throw error;
			else
				callback(null, row); //Un 1 o un 0, si sale mas de 1 es mala senyal
		})
	}
}*/
...
user.checkPassword = function(pw1, callback) {
	bcrypt.compare(pw1, pw2, function(err, isMatch) {
		if (err)
			throw err;
		else
